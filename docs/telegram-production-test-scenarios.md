...
## 시나리오 12. 최근 7일 공개 브리핑 recovery coverage / backfill

### 절차

1. `GET /api/cron/public-backfill?briefingSession=...&reportRunDate=...`를 누락된 recent session에 대해 호출하거나, 운영자가 `run:backfill-public-week`를 실행해 업로드 가능 기준일에서 최근 7일 구간만 다시 채운다.
...
4. public feed에서 recent recovery window earliest date가 계속 보이고, 이번 주 날짜 그룹과 `pre_market`, `post_market`, `weekend_briefing` 노출이 유지되는지 확인한다.
...
- public feed가 의도보다 적은 수의 row만 보이면 worker recent recovery backfill을 먼저 수행하고, `run:verify-public-week`로 DB와 detail HTML을 동시에 확인한다.
