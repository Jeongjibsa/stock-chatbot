...
- `run:verify-public-week`는 protected Vercel preview를 검증할 때 raw `curl`이 401/403을 반환하면 `vercel curl --deployment <preview-url>`로 자동 재시도해야 한다. preview 검증을 위해 protection을 임시 해제하는 방식은 기본값으로 사용하지 않는다.
