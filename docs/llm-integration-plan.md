...
이 문서는 텔레그램 입력과 일 배치 리포트 생성 과정에서 어떤 LLM API와 모델을 어떤 역할로 사용할지 정의한다.
...
- 2026-03-20 기준 LLM 호출 API 기준선은 `OpenAI Responses API`다.
...
1. `OpenAI client wrapper` 추가
2. `news-event-extraction` structured output 계약 추가
3. `news-summary` structured output 계약 추가
4. `market-report-composition` prompt 계약 추가
5. 결과 캐시, 실행 로그, prompt version 기록 연결
