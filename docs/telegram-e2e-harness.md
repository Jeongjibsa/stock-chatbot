...
- 시나리오 delta와 scope 분류는 `docs/e2e-change-workflow.md`를 기준으로 먼저 정리한다.
...
- `TELEGRAM_E2E_DATABASE_URL`
  - live production webhook를 검증할 때는 이 값을 production Neon connection string으로 주는 것을 권장한다.
  - 없으면 기본적으로 `DATABASE_URL`을 사용한다.
...
production webhook와 같은 DB를 기준으로 검증할 때:

```bash
TELEGRAM_E2E_DATABASE_URL='postgresql://...neon...' \
COREPACK_HOME=/tmp/corepack pnpm test:telegram:e2e -- --suite=minimum --allow-production
```
...
- 세션에 따라 `포트폴리오 리밸런싱 제안` 또는 `기준 보정 제안`이 보이는지
- `시세 스냅샷 연결 전입니다` placeholder가 남지 않는지
- 공개 링크 placeholder `확인 필요`나 legacy `/briefings/` 경로가 남지 않는지
...
- live production webhook을 칠 때는 webhook runtime이 바라보는 DB와 같은 connection string을 `TELEGRAM_E2E_DATABASE_URL`로 명시해야 outbound audit log와 side effect 검증이 맞는다.
