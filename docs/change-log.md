...
| CHG-0124 | 2026-03-24 | FIX | `/admin`의 Telegram 사용자 관리에서 block/unblock 제어를 운영용으로 마무리했다. admin write route는 성공/실패를 `/admin` 배너로 환류하고, 운영자 계정 `8606362482`는 화면과 route 양쪽에서 보호되며, 공용 `isAdminTelegramUserId` helper를 web/telegram runtime이 함께 사용한다. | Change Log, Context, Code, Tests | yes |
| CHG-0125 | 2026-03-24 | FIX | Telegram `/report`가 공개 브리핑 row를 찾지 못했을 때 legacy `/briefings/YYYY-MM-DD/...` fallback 링크를 붙이던 문제를 제거하고, 공개 링크는 `reports/[uuid]` row가 있을 때만 노출하도록 수정했다. 공개 전문 markdown는 실제 heading 구조로 생성하고, web renderer는 기존 숫자형 본문도 heading으로 보정해 가독성을 높였다. 또한 `apps/web`가 Vercel build에서 stale workspace `dist` 대신 source alias를 직접 번들링하도록 조정해 webhook/cron/public detail이 최신 공용 로직을 사용하게 했고, GitHub Actions `Daily Report`의 scheduled fallback trigger는 제거해 `workflow_dispatch` 기반 manual reconcile 전용으로 축소했다. | PRD, Plan, Change Log, Context, Code, Tests, Workflow | yes |
| CHG-0126 | 2026-03-25 | FIX | 정기 세션에서 공개 브리핑 업로드를 개인 발송보다 먼저 수행하도록 순서를 뒤집고, 공개 업로드가 만든 `reports/[uuid]` 링크를 scheduled daily report에 명시적으로 주입하도록 수정했다. 공개 업로드는 `10초 -> 20초` backoff로 최대 3회 시도하며, 끝내 링크를 확보하지 못하면 정기 브리핑은 보내되 공개 링크 섹션은 통째로 생략한다. Telegram renderer의 `확인 필요` placeholder도 제거했다. | PRD, Plan, Change Log, Context, Code, Tests, Ops | yes |
| CHG-0124 | 2026-03-24 | UPDATE | Telegram 입력 흐름과 운영 제어를 강화했다. 홈 `종목 추가`는 bulk-first 2-depth UX로 전환했고, `/portfolio_bulk`는 대화형 입력도 지원한다. `/portfolio_add`는 `종목 선택 -> 평균단가 여부 -> 평균단가 -> 수량 여부 -> 수량`으로 단순화하고 note 입력을 제거했다. 사용자별로 KST 일일 한도(`/report` 1회, `/portfolio_add`+`/portfolio_bulk` 합산 3회)와 flood block(1초 3회 초과 또는 10초 10회 초과)을 추가했으며, `/unregister`는 request history와 block 상태를 유지하는 soft reset으로 전환했다. 운영자 `telegram_user_id=8606362482`는 모든 제한에서 면제되고, `/admin`은 user block/unblock 제어와 오늘 사용량 조회를 지원한다. | PRD, Plan, Change Log, Context, Telegram Docs, Code, Tests, Ops | yes |
