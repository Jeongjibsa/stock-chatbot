...
텔레그램 기반 주식 챗봇을 구축한다. 현재 MVP는 사용자가 커맨드를 보낼 때마다 응답하는 방식에 더해, `07:30 pre_market / 20:30 post_market` 정기 브리핑을 보내는 세션형 스케줄 방식으로 동작한다. 주말 규칙은 미국장 기준으로 `토요일 오전까지만`, `토요일 오후/일요일 전체 skip`을 사용한다.
...
- 텔레그램 메시지 마지막에는 같은 날, 같은 `briefing_session`의 공개 웹 상세 브리핑 링크를 함께 제공한다.
...
- GitHub Actions workflow는 최소 `CI`, `Daily Report Reconcile(workflow_dispatch)`, `Telegram Smoke Test`를 유지한다.
- `Daily Report Reconcile` workflow는 Vercel Cron이 놓친 실행이나 운영자 수동 재실행만 담당해야 한다.
- `Daily Report Reconcile` workflow는 `workflow_dispatch`로만 실행하며, 가능하면 Vercel의 `/api/cron/reconcile` endpoint를 호출하는 manual reconcile mode를 우선 사용하고, 불가능할 때만 local worker fallback을 사용한다.
...
- `/api/cron/pre-market-briefing`와 `/api/cron/post-market-briefing`를 primary session cron route로 사용하고, `/api/cron/daily-report`는 현재 KST 기준 허용 세션만 실행하는 maintenance route로 유지한다.
- `/api/cron/reconcile`는 `briefingSession=pre_market|post_market|both`를 받아 세션별 보정 실행을 지원해야 한다.
