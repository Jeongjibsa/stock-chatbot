...
MVP 전달 채널은 `텔레그램 요약본 + GitHub Pages 상세 브리핑`의 이중 구조를 사용한다.
초기 전달 채널은 텔레그램이지만, 장기적으로는 동일한 코어 엔진을 기반으로 웹 앱과 모바일 앱까지 확장 가능한 구조를 목표로 한다.
현재 우선 구현 범위는 GitHub Pages 기반 공개 웹사이트까지이며, 모바일 앱은 후속 phase로 보류한다.

## 2. Product Goals

- 텔레그램 안에서 끝나는 투자 브리핑 경험 제공
- 텔레그램에서는 한눈에 읽히는 요약본을 제공하고, 상세한 공개 브리핑은 GitHub Pages로 제공
...
정기 브리핑은 사용자별 `enabled / hour / minute / timezone` 설정을 가질 수 있어야 하며, 스케줄러는 더 자주 실행되더라도 실제 발송은 각 사용자 설정에 맞는 시점에만 이뤄져야 한다.
...
- 텔레그램에는 개인화 정보가 포함된다.
- GitHub Pages에는 공개 가능한 공용 브리핑만 포함한다.
- `보유 종목별 최근 동향`과 `종목 관련 핵심 기사 및 이벤트 요약`은 개인별 특화 정보이므로 GitHub Pages 공개본에는 포함하지 않는다.
- 텔레그램 메시지 마지막에는 같은 날의 GitHub Pages 상세 브리핑 링크를 함께 제공한다.
- 공개 `채널` 메시지에는 GitHub Pages 링크와 공용 시장 브리핑만 포함하고, 개인화 섹션은 포함하지 않는다.
...
- GitHub Pages 상세 브리핑은 블로그형 정적 페이지로 생성하고, 공개 페이지에는 개인 식별 정보나 보유 종목 정보가 포함되지 않아야 한다.
- GitHub Pages 상세 브리핑 permalink는 canonical 경로 `/briefings/YYYY-MM-DD/`와 archive 경로 `/briefings/YYYY/MM/DD/`를 함께 지원하는 것을 기본 규칙으로 한다.
- GitHub Pages 상세 브리핑 정적 산출물은 build 단계에서 canonical과 archive 두 경로에 모두 생성돼야 한다.
...
- 초기 운영 환경에서는 비용 최소화를 위해 GitHub Actions scheduled workflow를 오전 9시 KST 근처 실행용 기본 스케줄러로 사용해야 함
- GitHub Actions 스케줄은 UTC cron 기준으로 정의하고, 기본 브랜치에서 실행되는 제약을 전제로 해야 함
- GitHub Actions schedule 지연 가능성을 고려해 저장 계층 idempotency와 지연 허용 문구를 함께 설계해야 함
...
- 텔레그램 메시지 하단에는 같은 날 GitHub Pages에 게시된 상세 브리핑 URL을 포함해야 함

### 7.5 Public Detail Briefing Experience

- GitHub Pages를 사용해 날짜별 상세 브리핑 페이지를 정적 사이트 형태로 게시해야 함
- GitHub Pages 게시물은 GitHub Actions workflow에서 자동 생성 및 배포되어야 함
- 공개 브리핑은 텔레그램 요약본보다 상세해야 하지만, 개인화 정보는 포함하지 않아야 함
- 페이지 URL은 일자 기준으로 안정적으로 생성되어 텔레그램에서 바로 링크할 수 있어야 함
- 같은 날짜에 재실행되더라도 동일 URL을 재배포하는 방식으로 idempotent하게 동작해야 함
- 공개 상세 브리핑은 블로그형 아카이브를 제공해 과거 브리핑을 날짜별로 조회할 수 있어야 함
- root `/`는 최신 공개 브리핑으로 진입시키고, `/briefings/`는 날짜별 archive index 역할을 해야 함
- GitHub Pages 생성물에는 `.nojekyll` 또는 GitHub Pages Actions 기반 배포 등 정적 배포에 필요한 설정이 포함되어야 함
...
- 채널 역할 분리는 아래를 기본 기준으로 한다.
  ...
  - GitHub Pages: 공개 가능한 시장/매크로/자금/이벤트 상세 브리핑 archive
  - future web/app: 인증 사용자용 포트폴리오 관리, 리포트 히스토리 조회, 설정 변경, 온디맨드 실행
...
- public GitHub repository의 무료 운영 이점을 활용하기 위해 초기 CI와 오전 9시 배치 트리거는 GitHub Actions를 우선 사용한다.
- 초기 운영은 "GitHub Actions가 먼저 공개 상세 브리핑을 생성·배포하고, 이어서 앱이 같은 날짜 permalink를 포함한 텔레그램 요약본을 생성·전달 가능한 상태로 만든다"는 구조를 기본값으로 한다.
...
- GitHub Actions workflow는 최소 `CI`와 `Daily Report` 두 종류를 유지한다.
- Telegram smoke test 자동화를 위해 추가 `workflow_dispatch` workflow를 유지할 수 있다.
- `Daily Report` workflow는 GitHub Actions 내부에서 직접 호출 가능한 전용 daily report runner 엔트리포인트를 사용해야 한다.
- `Daily Report` workflow는 `public briefing build -> Pages deploy -> daily report generate` 순서를 기본으로 하고, 같은 `runDate` 재실행 시 canonical(`/briefings/YYYY-MM-DD/`)과 archive(`/briefings/YYYY/MM/DD/`) 페이지를 덮어쓰며 root/index와 `/briefings/` archive index를 함께 재생성해야 한다.
- GitHub Actions의 시간 정확도와 job duration 한계가 제품 요구를 넘어서기 시작하면, 이후 phase에서 전용 worker/queue 인프라로 이관한다.
...
- `Supabase`는 추후 웹/앱 단계에서 `Auth`, `Storage`, `Realtime`, Edge Functions가 함께 필요해질 때 재검토하는 대안으로 둔다.
- 따라서 현재 우선순위는:
  - 로컬/테스트: Docker Compose PostgreSQL
  - 초기 운영 DB: Neon
  - 추후 앱/Auth/Storage 통합 플랫폼 후보: Supabase
