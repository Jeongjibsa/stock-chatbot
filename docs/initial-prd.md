...
- 초기 운영은 "GitHub Actions가 먼저 공개 상세 브리핑을 생성·배포하고, 이어서 앱이 같은 날짜 permalink를 포함한 텔레그램 요약본을 발송"하는 구조를 기본값으로 한다.
...
- `Daily Report` workflow는 `public briefing build -> Pages deploy -> daily report send` 순서를 기본으로 하고, 같은 `runDate` 재실행 시 canonical(`/briefings/YYYY-MM-DD/`)과 archive(`/briefings/YYYY/MM/DD/`) 페이지를 덮어쓰며 root/index와 `/briefings/` archive index를 함께 재생성해야 한다.
