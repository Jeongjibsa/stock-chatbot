...
공개 브리핑 `reports` 읽기 모델은 `2026-03-23` 기준일부터의 세션 데이터를 누락 없이 유지해야 하며, 운영 경로는 이 기준일 이후 missing row를 자동 복구해야 한다.
...
- `/api/cron/daily-report`와 `/api/cron/reconcile`는 현재 세션 실행 후 `2026-03-23` 이후 누락된 공개 브리핑 row를 다시 생성해 retained archive를 self-heal 해야 한다.
