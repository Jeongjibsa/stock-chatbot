...
텔레그램 기반 주식 챗봇을 구축한다. 현재 MVP는 사용자가 커맨드를 보낼 때마다 응답하는 방식에 더해, `07:30 pre_market / 20:30 post_market / 토요일 07:30 weekend_briefing` 세션형 스케줄 방식으로 동작한다. 주말 규칙은 미국장 기준으로 `월~금 오전 pre`, `월~금 오후 post`, `토요일 오전 weekend_briefing(public only)`, `일요일 전체 skip`을 사용한다.
...
공개 브리핑 `reports` 읽기 모델은 유실 발생 시 업로드 가능 기준일에서 최근 7일의 허용 세션 데이터를 다시 복구할 수 있어야 하며, 운영 경로는 이 rolling window 안의 missing row만 자동 복구해야 한다.
...
정기 브리핑은 채널별 깊이뿐 아니라 세션별 역할을 구분한다. 발송 시각은 `Asia/Seoul 07:30 pre_market`, `Asia/Seoul 20:30 post_market`, `토요일 07:30 weekend_briefing(public only)`으로 고정한다.

운영 캘린더:

- `월~금`: pre + post 둘 다 생성/발송
- `토`: `weekend_briefing` 공개 브리핑만 생성
- `일`: 정기/public 브리핑 없음
...
3. 브리핑 역할
  ...
  - `weekend_briefing`: `미장 마감 분석 및 주간 이슈 총정리, 다음 주 일정 요약`
4. 시장 종합 해석
...
- 공개 웹 feed/detail의 `핵심 시그널`은 `keyIndicatorBullets` structured output을 우선 사용해 생성해야 하며, LLM composition 실패 또는 빈 배열일 때만 규칙 기반 fallback을 허용한다.
- 공개 `핵심 시그널` fallback은 세션 목적에 맞게 `pre_market`, `post_market`, `weekend_briefing`별 후보를 다르게 사용해야 한다. threshold 기반 극단 신호가 적더라도 `pre/post`는 최소 2개, `weekend_briefing`은 최소 3개 시그널을 유지해야 하며, 부족한 경우에는 세션별 기본 관찰 포인트를 채워 넣어 feed 카드가 비어 보이지 않게 해야 한다.
- 공개 브리핑 LLM이 `keyIndicatorBullets`를 비우고 `marketBullets`, `macroBullets`, `riskBullets`, `eventBullets`, `trendNewsBullets`만 반환한 경우에는, public composition 경로가 이 bullet들을 우선 재조합해 `signals`를 채워 저장해야 한다. renderer의 generic placeholder만 보이고 DB `reports.signals`는 빈 배열로 남는 상태를 허용하지 않는다.
- 공개 브리핑 `summary` fallback은 직전 같은 세션의 `priorSignals`와 완전히 같은 첫 신호를 다시 복제하지 않도록 보정해야 한다. 최근 7일 feed에서 pre/post title이 날짜만 바뀐 채 반복되는 상태는 MVP 품질 기준에 맞지 않는다.
- public market relevance filter는 RSS 수집 단계뿐 아니라 historical regenerate에서 재사용하는 persisted `news_items` 분석 단계에도 다시 적용해야 한다. 따라서 생활형/개인재무성 기사, 단일 기업성 저신호 기사, duplicate reference는 final public headline/news reference에 남기면 안 된다.
- `핵심 뉴스 이벤트`는 `headlineEvents` structured output을 사용하되, 최종 공개본의 headline 아래 시장 해석 문장은 항상 한국어여야 한다. worker 저장 단계는 headline과 가장 가까운 테마/키워드를 기준으로 title-aligned Korean 시장 의미 문장을 다시 보정할 수 있어야 하며, 영어 RSS description이나 headline과 맞지 않는 theme summary를 그대로 노출하면 안 된다.
- historical public regenerate/backfill은 현재 시점 live RSS를 과거 `runDate` row에 그대로 덮어쓰면 안 된다. `runDate`가 현재 날짜보다 과거이면 먼저 DB에 저장된 해당 날짜의 `news_items`를 재사용하고, 그마저 없으면 live fetch 대신 뉴스 섹션을 비워 둔 채 시장 데이터 기반 요약만 유지해야 한다.
...
- 공개 브리핑의 `핵심 뉴스 이벤트`와 `거시 트렌드 뉴스`는 시장/금리/환율/정책/섹터 로테이션 같은 공개 시장 해석에 직접 필요한 기사만 사용해야 하며, 개인 재무 상담·은퇴·주거·카드·모기지 같은 생활형 기사나 가족 사연형 콘텐츠는 제외해야 함
- 공개 브리핑의 `핵심 뉴스 이벤트` summary는 headline 바로 아래 시장 의미를 설명하는 한 문장만 보여야 하며, `브리핑용 요약 제안` 같은 literal 라벨 문구는 노출하지 않아야 함
...
- `/api/cron/daily-report`와 `/api/cron/reconcile`는 현재 세션 실행 후 최근 7일 rolling window 안에서 누락된 공개 브리핑 row만 다시 생성해 self-heal 해야 한다.
- `briefingSession=both`는 현재 날짜 기준 허용 세션만 의미해야 한다. 즉 `월~금`은 `pre_market + post_market`, `토`는 `weekend_briefing`, `일`은 빈 세트여야 하며, `reportRunDate` 없이 토요일 current-date `pre_market` row나 일요일 current-date row를 만들면 안 된다.
...
3. 안내 내용에는 `07:30 pre_market(월~금)`, `20:30 post_market(월~금)`, `토요일 07:30 weekend_briefing(public only)`, `일요일 skip` 규칙을 포함한다.
