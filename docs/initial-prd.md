...
- 매일 1회 정기 브리핑이므로 `거시 시장 스냅샷`에는 별도 일자 라벨을 노출하지 않는다.
- 환율과 달러 인덱스는 `거시 시장 스냅샷` 하단에 연속 배치하고, 바로 아래에서 함께 해석한다.
- 시장 브리핑에는 `S&P500 / NASDAQ / KOSPI / KOSDAQ / DOW / VIX` 해석을 우선 반영한다.
- 매크로 브리핑에는 `미국 10년물 금리 / 기준금리 / CPI 발표 여부 / 달러 인덱스` 해석을 우선 반영한다.
- 자금 브리핑에는 `외국인 / 기관 순매수·순매도`와 `ETF flow` 요약을 우선 반영한다.
- 이벤트 브리핑에는 `주요 뉴스 3~5개 / 예정 실적 발표 일정 / 지정학 리스크 / AI·반도체·원자재 이슈`를 우선 반영한다.
- `시장 / 매크로 / 자금 / 이벤트`는 독립된 메타 카테고리처럼 보이기보다 PRD의 기존 브리핑 흐름 안에서 자연스럽게 이어지는 해설 섹션으로 렌더링한다.
...
- LLM 리포트 조합 출력은 텔레그램 렌더러가 요구하는 `한 줄 요약`, `시장`, `매크로`, `자금`, `이벤트`, `보유 종목`, `기사 요약`, `전략`, `리스크` 섹션과 직접 연결되어야 함
