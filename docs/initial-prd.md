...
텔레그램 기반 주식 챗봇을 구축한다. 현재 MVP는 사용자가 커맨드를 보낼 때마다 응답하는 방식에 더해, `07:30 pre_market / 20:30 post_market / 토요일 07:30 weekend_briefing` 세션형 스케줄 방식으로 동작한다. 주말 규칙은 미국장 기준으로 `토요일 오전 공개/정기 생성 허용`, `토요일 오후/일요일 전체 skip`을 사용한다.
...
- `briefingSession=both`는 현재 날짜 기준 허용 세션만 의미해야 한다. 즉 `월~금`은 `pre_market + post_market`, `토`는 `pre_market + weekend_briefing`, `일`은 빈 세트여야 하며, `reportRunDate` 없이 일요일 current-date row를 만들면 안 된다.
