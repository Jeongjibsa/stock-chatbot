...
Telegram `/report`와 공개 브리핑의 제목 날짜 및 공개 `report_date`는 요청 시각이 아니라 서울 기준 `공통 마감일(effective report date)`을 사용해야 한다. 즉, 국장과 미장의 대표 지표에서 확인된 가장 최근 마감일을 기준으로 같은 날짜의 브리핑을 재사용해야 한다.
개인화 리밸런싱 payload는 `user_id + effective_report_date` 키의 precomputed snapshot cache를 우선 사용하고, cache가 없거나 stale하면 새 snapshot을 생성한 뒤 재사용해야 한다.
...
- 운영/테스트에서는 `REPORT_RUN_DATE=YYYY-MM-DD` override로 `/report` 기준일을 고정해 특정 거래일 실데이터를 재현할 수 있어야 한다.
