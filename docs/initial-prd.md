...
텔레그램 기반 주식 챗봇을 구축한다. 초기 MVP는 사용자가 커맨드를 보낼 때마다 응답하는 방식이 아니라, 매일 오전 8시에 한 번 자동으로 개인화된 리포트를 보내는 일 배치 스케줄 방식으로 동작한다. 이후 단계에서 `/report` 같은 온디맨드 커맨드 요청을 추가한다.
...
1. 사용자는 매일 오전 8시에 오늘 시장 상황과 내 종목 관련 이슈를 자동으로 받고 싶다.
...
초기 MVP의 중심은 여전히 매일 오전 8시 스케줄 발송이지만, 현재 구현 기준으로는 `DM`에서 `/report` 온디맨드 호출도 지원한다.
...
6. 선택된 종목에 대해 기존 평균단가/수량/메모 입력 플로우를 이어간다.
...
- `/unregister`는 기존 등록 정보와 개인 설정을 초기화하는 명령으로 제공해야 하며, 초기화 후 다시 `/register`로 온보딩을 재시작할 수 있어야 한다.
...
오전 8시 자동 발송 리포트는 채널별로 다른 깊이를 사용한다.

텔레그램 요약본:

제목은 `오늘의 포트폴리오 리밸런싱 브리핑 (YYYY-MM-DD)` 형식을 사용한다.
...
- 텔레그램 개인화 브리핑은 `hard rule -> final action -> 시장 레짐 -> 3대 관점 -> 기타 사실` 순서로 읽히게 구성한다.
- `오늘의 리밸런싱 제안`은 `비중 확대 검토 / 유지 우세 / 비중 조절 필요 / 우선 관찰 대상` 4개 라인을 항상 채운다.
...
- production 기본 스케줄러는 Vercel Cron을 사용해야 하며, `오전 8시 KST`는 UTC cron으로 변환해 설정해야 함
...
- `/portfolio_add`, `/portfolio_remove`, `/report_time`는 명령 시작 후 단계별 상태를 유지하는 대화형 플로우로 동작해야 함
- DM에서는 `/start`, `/help`, `/register` 이후 홈 reply keyboard를 노출해 `브리핑 보기 / 종목 추가 / 내 종목 / 설정` 흐름을 버튼으로 바로 진입할 수 있어야 함
- `/report_settings`는 inline keyboard로 `브리핑 켜기/끄기`, `밀도 변경`, `상세 링크 표시/숨김`, `시간 변경 안내`를 제공해야 하며 기존 slash command semantics는 유지해야 함
...
- 개인화 `/report`는 `오늘 한 줄 결론 -> 오늘의 리밸런싱 제안 -> 성향별 해석 -> 시장 레짐 요약 -> 종목별 리밸런싱 가이드` 순서의 리밸런싱 브리핑 템플릿을 사용해야 함
...
- 초기 무료 운영 단계의 오전 배치 스케줄은 "오전 8시 KST 근처에 1회 실행"을 목표로 하며, exact 8:00 보장은 요구하지 않는다.
...
### Daily 08:00 Report Flow

1. 매일 오전 8시 스케줄 트리거 실행
2. 리포트 대상 사용자 조회
3. 사용자 설정과 포트폴리오 조회
...
1. 사용자가 `/report_settings`의 `시간 변경` 버튼을 누르거나 `/report_time HH:MM`을 직접 입력
2. 버튼 경로에서는 시스템이 `HH:MM` 입력을 기다리는 대화 상태로 전환
3. 사용자가 시간을 입력하면 유효성 검사를 수행
4. 저장 후 새 발송 시간을 다시 안내
...
- `daily-report-orchestrator`
  - 오전 8시 스케줄 실행, 대상 사용자 반복 처리, 재시도 규칙 적용
- `market-snapshot`
...
- scheduler 또는 orchestrator가 오전 8시 작업을 시작
...
- 오전 8시 배치 MVP를 가장 빠르게 만들고 싶은 경우
...
- 오전 8시 배치로 시작하되 퀀트 로직을 지속적으로 키울 계획이 있는 경우
...
- 사용자 수가 적고 오전 8시 일 배치만 먼저 빠르게 돌리고 싶은 경우
...
- 현재 MVP는 오전 8시 하루 1회 자동 발송이므로 시스템 복잡도를 낮추는 것이 더 중요하다
...
- 오전 8시 일 배치 리포트는 worker와 queue로 처리
