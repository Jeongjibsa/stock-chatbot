...
- 보유 종목별 최근 동향은 가능하면 `runDate` 기준 최근 가용 거래일의 `전일 종가 → 현재가`와 등락률을 함께 보여준다.
- `퀀트 기반 시그널 및 매매 아이디어`는 시작 부분에 `비중 확대 검토 / 유지 우세 / 비중 조절 필요 / 우선 관찰 대상` 형태의 리밸런싱 요약을 먼저 보여준다.
- 그 아래에는 보유 종목별 `Macro / Trend / Event / Flow -> Total -> Action` 형식의 점수카드와 행동 제안 bullet을 붙인다.
