...
- `/portfolio_add`, `/portfolio_bulk`, `/portfolio_remove`, `/portfolio_list`, `/market_add`, `/market_items`는 등록된 사용자만 사용할 수 있어야 한다.
...
1. 사용자가 항목 이름 입력
2. 시스템이 지표 alias registry와 코드 정규화 규칙으로 지원 가능한 지표 매핑 시도
3. 확인 후 저장
