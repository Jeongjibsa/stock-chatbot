...
- PR을 열거나 갱신할 때는 제목 외 본문(summary, validation, ops note, checklist 포함)을 한글과 영문 두 버전으로 함께 작성한다.
- PR merge가 완료되면 해당 feature/fix 로컬 브랜치는 `git branch -d <branch>`로 정리한다.
