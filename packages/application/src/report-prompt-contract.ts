    ...
    return [
      ...
      briefingSession === "pre_market"
        ...
        : briefingSession === "post_market"
          ...
          : "현재 세션은 주말 브리핑이다. 가장 최근 미장 마감 분석을 바탕으로 주간 이슈를 압축하고 다음 주 주요 일정을 정리하는 데 집중한다.",
      ...
      briefingSession === "weekend_briefing"
        ? "keyIndicatorBullets는 공개 feed 카드의 `핵심 시그널`에 바로 쓸 수 있게 작성한다. 시장에서 지금 가장 먼저 읽어야 할 신호를 최대 4개 독립 문장으로 짧게 정리하고, 주말 브리핑에서는 최소 3개를 채운다. 입력 시장 데이터가 있는데 빈 배열로 두지 않는다."
        : "keyIndicatorBullets는 공개 feed 카드의 `핵심 시그널`에 바로 쓸 수 있게 작성한다. 시장에서 지금 가장 먼저 읽어야 할 신호를 최대 4개 독립 문장으로 짧게 정리하고, 공개 브리핑에서는 최소 2개를 채운다. 입력 시장 데이터가 있는데 빈 배열로 두지 않는다.",
      ...
      briefingSession === "post_market"
        ...
        : briefingSession === "weekend_briefing"
          ? "주말 브리핑에서는 가장 최근 미장 마감 흐름, 주중 반복된 테마, 다음 주 선행 체크포인트를 우선 정리한다."
